...
mc_df = pd.DataFrame(columns=['id', 'mass_center_x', 'mass_center_y', 'max_x', 'min_x', 'max_y', 'min_y', 'box_centre_x', 'box_centre_y', 'point_count'])
new_id = 0
for i, id in enumerate(unique_merged_ids):
	...
	new_clust = pd.DataFrame({'id': [new_id], 'mass_center_x': [centre_x], 'mass_center_y': [centre_y], 'max_x': [max_x], 'min_x': [min_x], 
						   'max_y': [max_y], 'min_y': [min_y], 'box_centre_x': [box_centre_x], 'box_centre_y': [box_centre_y], 'point_count': [total_mass]})
...
axs[2].scatter(mc_df['mass_center_x'], mc_df['mass_center_y'], c=mc_df['id'], cmap='turbo', alpha=0.6, edgecolors='k')  # Marking the centers
