		...
		for new_obj in new_objects:
			...
			for obj in self.all_tracked_objects:
				...
				if dt < 0.8: # then not new object
					continue
				insideBBOX: bool = ((new_obj.xpos < obj.brx) and 
				(new_obj.xpos > obj.tlx) and 
				(new_obj.ypos > obj.bry) and (new_obj.ypos < obj.tly))

				area_diff_check: bool = (abs(new_obj.area - obj.area) / new_obj.area) < 0.25

				mass_center_dist_sqrd_check = ((new_obj.xpos - obj.xpos)**2 + (new_obj.ypos - obj.ypos)**2) < 9.0  # check of masscenter is within 3 meter of last seen, dist squared
				
				alt_center_dist_check =  ((new_obj.xpos - obj.xpos)**2 + (new_obj.ypos - obj.ypos)**2) < 12**2
				# TODO: new matching that can replace inside bbox, if areas are similar and within M meters like 10, then it could be object that starts to move. them match
				# checks:
				# if insideBBOX:
					# if not area_diff_check:
					# 	print(f"areaDiffCheck NOT valid: {abs(new_obj.area - obj.area) / new_obj.area}")

					# if not mass_center_dist_sqrd_check:
					# 	print(f"massCenterDistSqrdCheck NOT valid: {((new_obj.xpos - obj.xpos)**2 + (new_obj.ypos - obj.ypos)**2)}")

				# if insideBBOX and area_diff_check and mass_center_dist_sqrd_check:  # area can shrink/grow 25%
				if area_diff_check and alt_center_dist_check: # area can shrink/grow 25%
					match = True
		...
		for delete_id in to_delete:
			for index, obj in enumerate(self.all_tracked_objects):
				if delete_id == obj.id:
					...
					self.all_tracked_objects.pop(index)
