...
int main(int argc, char* argv[]) {
	...
	for (int r=0; r<ti+1; r++) {
		...
		auto duration = std::chrono::duration_cast<std::chrono::duration<double>>(syst_cend - syst_t0);
		std::cout << "Clustering time: " << std::fixed << std::setprecision(4) << duration.count() << " seconds" << std::endl;
